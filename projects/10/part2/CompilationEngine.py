...
class CompileEngine(object):
    ...
    def _CompileSubroutineBody(self):
        ...
        while True:
            if self.tokenizer.TokenType() != TK_KEYWORD:
                break
            if self.tokenizer.Keyword() != KW_VAR:
                break
            self._CompileVarDec()
    ...
    def _CompileCall(self, tokenName=None):
        ...
        def process_call():
            # Process subroutine of form FUNCTION.SUBROUTINE()
        ...
        # If identifier already processed
        if tokenName:
            self._WriteXml('identifier', tokenName)
            self._ExpectSymbol('.(')
        else:
            #  No identifier processed, check for subroutine|class|var name
            self._ExpectIdentifier()
            self._WriteXml('identifier', self.tokenizer.Identifier())
            self._NextToken()
    ...
    def _CompileIf(self):
        ...
        if self.tokenizer.TokenType() == TK_KEYWORD and self.tokenizer.KeywordStr() == "else":
