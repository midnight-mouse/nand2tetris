...
class CompileEngine(object):
    ...
    def _CompileCall(self, subroutineName=None):
        ...
        if objectName:
            # check if objectName in symboltable
            if objectName in self.symbolTable.subroutineSymbols:
                # Get variable information
                variableKind = self.symbolTable.KindOf(objectName)
                variableIndex = self.symbolTable.IndexOf(objectName)
                variableSegment = self._KindToSegment(variableKind)

                # Push variable on stack and increase arg count
                self.vmWriter.WritePush(variableSegment, variableIndex)
                argCount += 1

                # Update objectName to reflect the object being called
                objectName = self.symbolTable.subroutineSymbols[objectName][0]

            # Evaluate subroutine expressions and count arguments
            argCount += self._CompileExpressionList()

            # Call subroutine
            self.vmWriter.WriteCall(f"{objectName}.{subroutineName}", argCount)
        else:
            ...
            self.vmWriter.WriteCall(f"{self.className}.{subroutineName}", argCount)
    ...
    def _CompileIf(self):
        ...
        if self.tokenizer.TokenType() == TK_KEYWORD and self.tokenizer.Keyword() == KW_ELSE:
            ...
            # TRUE statement jump over else
            self.vmWriter.WriteGoto(contLabel)

            # End if statement
            self.vmWriter.WriteLabel(endIfLabel)
            
            # Else will continue past end of if statement
            ...
            # Write continue point for TRUE section
            self.vmWriter.WriteLabel(contLabel) 
        else:
            # If no else, end if statement
            self.vmWriter.WriteLabel(endIfLabel)
